...
class BookListViewTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        #Create 11 books for pagination test
        number_of_books = 11

        # Create 11 authors
        for author_id in range(number_of_books):
            # Generate a random name using the module names
        ...
        # Create some languages and genres
        languages = ["English", "French", "Chinese", "Italian", "Spanish"]
        ...
        for n in range(number_of_books):
            Book.objects.create(
                ...
                language = Language.objects.get(pk=random.randint(1, len(languages))
                    ),
                # Randomly select an author from the Author database
                author = Author.objects.get(pk=random.randint(1, 11)),
                # Generate a random isbn of 13 digits
                isbn = ''.join(str(random.randint(0, 9)) for _ in range(13))
            )

        # Add genres to books. Since genre is a many-to-many field, direct assignment is not allowed. It has to be done with set method
        for book in Book.objects.all():
            # Select 1 to 3 random generes for each book
            genre_objects = Genre.objects.filter(pk__in=random.sample(range(len(genres)), random.randint(1,3)))
    ...
    def test_get_context_data_function(self):
        response = self.client.get(reverse('books'))
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def setUpTestData(cls):
        # Create test users
        users = {'testuser1':'1X<ISRUkw+tuK', 'testuser2':'2HJ1vRV0Z&3iD'}
        for username in users.keys():
            User.objects.create_user(username=username, password=users[username])

        # Give test_user2 permission to renew books.
        permission = Permission.objects.get(name='Set book as returned')
        User.objects.get(username="testuser2").user_permissions.add(permission)
...
#        test_user2.save()
        ...
        test_bookinstance1 = BookInstance.objects.create(
            book=test_book,
            imprint='Unlikely Imprint, 2016',
            due_back=return_date,
        ...
        return_date = datetime.date.today() + datetime.timedelta(days=5)
        test_bookinstance2 = BookInstance.objects.create(
            book=test_book,
            imprint='Unlikely Imprint, 2016',
            due_back=return_date,
            borrower=User.objects.get(username="testuser2"),
            status='o',
        )
    ...
    def setUp(self):
        ...
        test_book = Book.objects.get(isbn="ABCDEFG")
        self.test_bookinstance1 = BookInstance.objects.get(book=test_book, borrower=test_user1)
        self.test_bookinstance2 = BookInstance.objects.get(book=test_book, borrower=test_user2)
...
class AuthorCreateViewTest(TestCase):
    ...
    def test_logged_in_with_correct_permission(self):
        ...
        self.assertEqual(list(response.context['form'].fields.keys()), ['first_name', 'last_name', 'date_of_birth', 'date_of_death'])
        self.assertEqual(response.context['form'].initial['date_of_death'], '11/06/2020')

    def test_redirect_after_author_form_submitted_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.post(reverse('author-create'), {'first_name':'John', 'last_name':'Frank', 'date_of_birth':'1940-02-21', 'date_of_death':'2020-11-23'})
